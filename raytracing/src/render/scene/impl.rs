...
impl Scene {
    ...
    fn get_color_of_ray(&self, ray: &Ray, remaining_depth: u32) -> Vec3f {
        ...
        match self.intersect_with_first_object(ray) {
            ...
            Some((dist, object)) => {
                ...
                let refracted_ray = Self::refract(
                ...
                for light in &self.lights {
                    let light_dir_normalized = (light.position() - intersection_point).normalized();

                    if self
                        .intersect_with_first_object(&Ray::new(
                            intersection_point,
                    ...
                    diffuse_light_intensity +=
                        light.intensity() * light_dir_normalized.dot(unit_normal).max(0.0);
                    specular_light_intensity += light.intensity()
                        * Self::reflect(light_dir_normalized, unit_normal)
                            .dot(ray.direction_normalized())
                            .max(0.0)
    ...
    fn intersect_with_type<'a, T: 'a + SceneObject>(
        &self,
        ray: &Ray,
        obj_iter: impl Iterator<Item = &'a T>,
    ) -> Option<(Float, &'a dyn SceneObject)> {
        obj_iter
            .filter_map(|b| b.distance_to_intersection(ray).map(|c| (c, b)))
            .min_by(|(c1, _), (c2, _)| c1.partial_cmp(c2).unwrap())
    ...
    fn refract(v_normalized: Vec3f, unit_normal: Vec3f, eta: Float) -> Option<Vec3f> {
        ...
        if k < 0.0 {
            ...
            Some(eta * v_normalized + (eta * cos_i - k.sqrt()) * unit_normal)
        }
    }
}
